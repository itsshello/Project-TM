...
def get_songs(music_directory, album):  album_path = os.path.join(music_directory, album); return [os.path.join(album_path, f) for f in os.listdir(album_path) if f.endswith(('.mp3'))]
...
def searchBar_edit_on_change(searchBar_edit, new_edit_text): 
    searchBar_edit.set_caption("   Search") if new_edit_text == "" else searchBar_edit.set_caption("")
...
# Buttons
player_Controls_Suffel    = urwid.Button("[S]");      urwid.connect_signal(player_Controls_Suffel,    'click',  playerControls_Suffel)
player_Controls_Previouse = urwid.Button("[P]");      urwid.connect_signal(player_Controls_Previouse, 'click',  playerControls_Previouse)
...
player_Controls_Repeat    = urwid.Button("[R]");      urwid.connect_signal(player_Controls_Repeat,    'click',  playerControls_Repeat)
